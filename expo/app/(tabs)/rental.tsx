...
export default function RentalScreen() {
  ...
  const rentalItems = catalogByArea('Locação');

  const handleRequestRental = async (item: CatalogItem) => {
    if (user?.type === 'client') {
      const conversationId = await createConversation(
        'Locação',
        `Solicito locação: ${item.name}`
      );
      ...
      <View style={styles.content}>
        ...
        {item.price && (
          <Text style={styles.price}>
            R$ {item.price.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}/dia
          </Text>
        )}
        {user?.type === 'client' && (
          <Pressable
          ...
          >
            ...
            <Text style={styles.buttonText}>Solicitar Locação</Text>
          </Pressable>
        )}
      </View>
    </View>
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.subtitle}>Equipamentos para aluguel</Text>
        </View>
        <Image
